...
void __init e820__memory_setup(void)
{
	...
	who = x86_init.resources.memory_setup();         /*************************************************************memory op*********************************************************/
        /*x86_init.resources.send_printklog();*/

	memcpy(e820_table_kexec, e820_table, sizeof(*e820_table_kexec));

        /*printk("afer the first copy");
        x86_init.resources.send_printklog();*/

	memcpy(e820_table_firmware, e820_table, sizeof(*e820_table_firmware));
        /*printk("after the second copy");
        x86_init.resources.send_printklog();*/


	pr_info("e820: BIOS-provided physical RAM map:\n");
	e820__print_table(who);
        /*printk("after the pr info ");
        x86_init.resources.send_printklog();*/
...
void __init e820__memblock_setup(void)
{
	...
	for (i = 0; i < e820_table->nr_entries; i++) {
		...
		if (entry->type != E820_TYPE_RAM && entry->type != E820_TYPE_RESERVED_KERN)
			continue;
                printk( " \n adding the entry i  = %d, [%llu-%llu], in the memory", i, entry->addr, end);
		memblock_add(entry->addr, entry->size);
	}
        printk( " \n last entry i  = %d, end %llu, in the memory",  i,  end); 
        printk(" after adding the memory ");
        /*x86_init.resources.send_printklog();*/
	/* Throw away partial pages: */
	memblock_trim_memory(PAGE_SIZE);
	memblock_dump_all();
        printk(" after dumping the memory ");                         
        printk(" after dumping the memory ");                        
        x86_init.resources.send_printklog();
