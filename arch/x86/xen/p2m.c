...
void xen_setup_mfn_list_list(void)
{
	...
	HYPERVISOR_shared_info->arch.max_pfn = xen_p2m_last_pfn;
        printk("Lavoisier wapet in function %s ; setting the HYPERVISOR_shared_info  xen_p2m_last_pfn: %lu",__func__, xen_p2m_last_pfn );
	...
	for (pfn = 0; pfn < xen_max_p2m_pfn; pfn += chunk) {
		...
		chunk = (pfn & (P2M_PER_PAGE * P2M_MID_PER_PAGE - 1)) ?
			P2M_PER_PAGE : P2M_PER_PAGE * P2M_MID_PER_PAGE;
                /* need to create a new block or not*/
...
void __init xen_vmalloc_p2m_tree(void)
{
	...
	p2m_limit = (phys_addr_t)P2M_LIMIT * 1024 * 1024 * 1024 / PAGE_SIZE;//number of pages 
	vm.flags = VM_ALLOC;
	vm.size = ALIGN(sizeof(unsigned long) * max(xen_max_p2m_pfn, p2m_limit),
			PMD_SIZE * PMDS_PER_MID_PAGE);
        printk("Lavoisier wapet in function %s , initialisation of the p2m virtual memory (of the second type), p2m_limit : %lu ,  vm.size : %lu , xen_max_p2m_pfn : %lu ",__func__,p2m_limit,vm.size,xen_max_p2m_pfn);
	vm_area_register_early(&vm, PMD_SIZE * PMDS_PER_MID_PAGE);
        printk(" \n p2m virtual  addreess : %llu", (u64)(vm.addr));
	pr_notice("p2m virtual area at %p, size is %lu \n", vm.addr, vm.size);
