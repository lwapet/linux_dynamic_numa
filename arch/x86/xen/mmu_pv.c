...
static void xen_set_pmd_hyper(pmd_t *ptr, pmd_t val)
{
	...
	/* ptr may be ioremapped for 64-bit pagetable setup */
        ...
        /*printk(" \n Lavoisier in function %s, before arbitrary_virt_to_machine of %llu, address of pmd",__func__,(unsigned long long) ptr);
        printk(" \n Lavoisier in function %s, before arbitrary_virt_to_machine of %llu, address of pmd",__func__,(unsigned long long) ptr);
        x86_init.resources.send_printklog();*/

	u.ptr = arbitrary_virt_to_machine(ptr).maddr;

        /*printk(" \n Lavoisier in function %s, before pmd_val_ma of %lu, address of pmd",__func__, val.pmd);
         printk(" \n Lavoisier in function %s, before pmd_val_ma of  %lu, address of pmd",__func__, val.pmd );
        x86_init.resources.send_printklog();*/


	u.val = pmd_val_ma(val);
        /*printk(" \n before the xen_extend_mmu_update");
        printk(" \n before the xen_extend_mmu_update");
        x86_init.resources.send_printklog();*/
	...
	xen_extend_mmu_update(&u);
        ...
        /*printk(" \n after the xen_extend_mmu_update");
        printk(" \n after the xen_extend_mmu_update");
        x86_init.resources.send_printklog();*/
...
static bool xen_batched_set_pte(pte_t *ptep, pte_t pteval)
{
	...
	xen_mc_batch();
/*
           printk("\n in function %s, before modification of mmu",__func__);
                printk("\n in function %s, before modification of mmu",__func__);
                x86_init.resources.send_printklog();
*/
  ...
  /*      printk("\n in function %s, after modification of mmu",__func__);
                printk("\n in function %s, after modification of mmu",__func__);
                x86_init.resources.send_printklog();
*/
...
static void __init pin_pagetable_pfn(unsigned cmd, unsigned long pfn)
{
	...
	op.cmd = cmd;
	op.arg1.mfn = pfn_to_mfn(pfn);
        /*printk("\nbefore HYPERVISOR_mmuext_op on the mfn %lu containing the pte allocated",op.arg1.mfn );
        printk("\nbefore HYPERVISOR_mmuext_op on the mfn %lu containing the pte allocated",op.arg1.mfn );
        x86_init.resources.send_printklog();*/
	if (HYPERVISOR_mmuext_op(&op, 1, NULL, DOMID_SELF))
		BUG();

        /*printk("\nafter the  HYPERVISOR_mmuext_op on the mfn %lu containing the pte allocated",op.arg1.mfn );
        printk("\nafter the  HYPERVISOR_mmuext_op on the mfn %lu containing the pte allocated",op.arg1.mfn );
        x86_init.resources.send_printklog();*/
...
{
        printk("\nLavoisier wapet in function : %s  for page tables initialisation", __func__);
	...
	paging_init();     
        ...
        /* printk("\nbefore xen_post_allocator_init");*/
	xen_post_allocator_init();///////////////////// initialisation of memory manipulation functions
        printk("\n after xen_post_allocator_init");
        printk("\n after xen_post_allocator_init");
        send_printklog_tovmm();
	xen_pagetable_p2m_setup();
        printk("\n after xen_pagetable_p2m_setup");
        printk("\n after xen_pagetable_p2m_setup");
        send_printklog_tovmm(); 
	/*Allocate and initialize top and mid mfn levels for p2m structure */
	xen_build_mfn_list_list();
        printk("\n after xen_build_mfn_list_list");
        printk("\n after xen_build_mfn_list_list");
        send_printklog_tovmm();
	/* Remap memory freed due to conflicts with E820 map */

	xen_remap_memory(); ///////////// remapping memory
        printk("\n after xen_remap_memory");
        printk("\n after xen_remap_memory");
        send_printklog_tovmm();
	xen_setup_shared_info();
        printk(" \n after xen_setup_shared_info");
        send_printklog_tovmm();

        /**** trying to remvove iside this *********/
}
...
#endif
        /*printk("\nbefore make_lowmem_page_readonly on the mfn containing the pte allocated");
        printk("\nbefore make_lowmem_page_readonly on the mfn containing the pte allocated");
        x86_init.resources.send_printklog();*/
	make_lowmem_page_readonly(__va(PFN_PHYS(pfn)));
        /* printk("\nbefore pin_pagetable_pfn on the mfn containing the pte allocated");
        printk("\nbefore pin_pagetable_pfn on the mfn containing the pte allocated");
        x86_init.resources.send_printklog();*/
	pin_pagetable_pfn(MMUEXT_PIN_L1_TABLE, pfn);
        /* printk("\nafter pin_pagetable_pfn on the mfn containing the pte allocated");
        printk("\nafter pin_pagetable_pfn on the mfn containing the pte allocated");
        x86_init.resources.send_printklog();*/
}
...
{
        /*the page table address has to be present in the e820 map before continue*/
